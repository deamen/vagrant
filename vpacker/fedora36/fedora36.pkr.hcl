...
source "virtualbox-iso" "fedora36" {
  ...
  iso_url      = "${local.iso_url}"
  iso_checksum = "${local.iso_checksum}"
  # Guest additons are included in fedora now
...
build {
  ...
  # Install virtualbox geust addtions
  provisioner "shell" {
    inline = [
      "sudo dnf install -y virtualbox-guest-additions nfs-utils",
    ]
    only = [
      "virtualbox-iso.fedora36",
    ]
  }
